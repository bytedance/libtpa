...
static void dump_flow_action(const struct rte_flow_action *action)
{
	...
	} else if (action->type == RTE_FLOW_ACTION_TYPE_VF) {
		const struct rte_flow_action_vf *vf  = action->conf;

		LOG_DEBUG("rte flow vf action:");
		if (vf) {
			LOG_DEBUG("  vf original=%d", vf->original);
		} else {
			LOG_DEBUG("  null");
		}
...
static void add_vf_action(struct offload_ctx *ctx)
{
	ctx->vf.original = 1;
	add_flow_action(&ctx->actions, RTE_FLOW_ACTION_TYPE_VF, &ctx->vf);
}
...
{
	/* append a VF action, which is needed for iavf */
	add_vf_action(ctx);
}
